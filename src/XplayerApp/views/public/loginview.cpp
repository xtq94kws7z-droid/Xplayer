...
LoginView::LoginView(XplayerCore *core, QWidget *parent)
    ...
    if (rememberServer && !lastServerId.isEmpty()) {
      ...
      if (serverExists) {
        ...
        QTimer::singleShot(0, this, [this, lastServerId]() {
          ...
          QTimer::singleShot(XplayerUi::kAutoLoginDelayMs, this, [this, lastServerId]() {
...
void LoginView::applyResponsiveLayout() {
  const qreal scale = XplayerResponsiveUtils::scaleForViewport(size());
  if (m_pageSwitcher) {
    m_pageSwitcher->setFixedWidth(
        XplayerResponsiveUtils::scaled(360, scale, 300, 520));
  }
  if (m_wheelView) {
    m_wheelView->setFixedSize(
        XplayerResponsiveUtils::scaled(340, scale, 280, 492),
        XplayerResponsiveUtils::scaled(220, scale, 172, 319));
  }
  const int footerButtonSize =
      XplayerResponsiveUtils::scaled(22, scale, 20, 32);
  const int footerIconSize =
      XplayerResponsiveUtils::scaled(14, scale, 12, 20);
  if (m_cloudSyncBtn) {
    m_cloudSyncBtn->setFixedSize(footerButtonSize, footerButtonSize);
    m_cloudSyncBtn->setIconSize(QSize(footerIconSize, footerIconSize));
  }
  if (m_footerSpacer) {
    m_footerSpacer->setFixedSize(footerButtonSize, footerButtonSize);
  }
  if (m_protocolInput) {
    m_protocolInput->setFixedWidth(qMax(
        XplayerResponsiveUtils::scaled(126, scale, 112, 180),
        m_protocolInput->sizeHint().width()));
  }
  if (m_portInput) {
    m_portInput->setMinimumWidth(
        XplayerResponsiveUtils::scaled(110, scale, 96, 160));
  }
  if (m_serverProxyBtn) {
    m_serverProxyBtn->setFixedWidth(
        XplayerResponsiveUtils::scaled(36, scale, 32, 52));
    m_serverProxyBtn->setIconSize(
        QSize(XplayerResponsiveUtils::scaled(18, scale, 16, 26),
              XplayerResponsiveUtils::scaled(18, scale, 16, 26)));
  }
}
...
void LoginView::refreshServerList() {
  ...
  auto createAddCardNode = [this, scale]() {
    ...
    addCardLayout->setContentsMargins(
    ...
    const int addIconSize = XplayerResponsiveUtils::scaled(40, scale, 34, 58);
    addIconLabel->setFixedSize(addIconSize, addIconSize);
    ...
    addInfoLayout->setSpacing(XplayerResponsiveUtils::scaled(2, scale, 1, 4));
    ...
    nFont.setPixelSize(XplayerResponsiveUtils::scaled(15, scale, 13, 22));
    nFont.setBold(true);
    ...
    uFont.setPixelSize(XplayerResponsiveUtils::scaled(12, scale, 10, 17));
    ...
    addCardLayout->addSpacing(XplayerResponsiveUtils::scaled(12, scale, 9, 18));
  ...
  for (int r = 0; r < repeatCount; ++r) {
    ...
    for (const auto &server : servers) {
      ...
      const int iconSize = XplayerResponsiveUtils::scaled(40, scale, 34, 58);
      ...
      infoLayout->setSpacing(XplayerResponsiveUtils::scaled(2, scale, 1, 4));
      ...
      actionLayout->setSpacing(XplayerResponsiveUtils::scaled(8, scale, 6, 12));
      ...
      cardLayout->addSpacing(XplayerResponsiveUtils::scaled(12, scale, 9, 18));
