...
QString routeKeyForWidget(const QWidget *widget)
{
    if (!widget) {
        return {};
    }
    ...
    return QStringLiteral("%1|%2|%3")
        .arg(widget->property("routeType").toString(),
             widget->property("routeId").toString(),
             widget->property("routeExtraId").toString());
}

QString routeKeyForInfo(const RouteInfo &info)
{
    return QStringLiteral("%1|%2|%3")
        .arg(info.routeType, info.routeId, info.routeExtraId);
}
} 

HomeView::HomeView(XplayerCore *core, QWidget *parent) : QWidget(parent), m_core(core)
{
...
QWidget *HomeView::createDetailView(const QString &itemId, const QString &itemName, const MediaItem &seedItem)
{
    ...
    view->setProperty("preferSnapshotTransition", true);
    view->setProperty("deferTeardownForTransition", true);
...
QWidget *HomeView::createCategoryView(const QString &categoryId, const QString &title)
{
    ...
    view->prepareCategory(categoryId, title);
    const QPointer<CategoryView> safeView(view);
    scheduleDeferredInitialLoad(view, [safeView]() {
        if (!safeView) {
            return;
        }
        QCoro::connect(safeView->loadPreparedCategory(), safeView.data(), []() {});
    });
...
QWidget *HomeView::createLibraryView(const QString &libraryId, const QString &title)
{
    ...
    view->prepareLibrary(libraryId, title);
    const QPointer<LibraryView> safeView(view);
    scheduleDeferredInitialLoad(view, [safeView]() {
        if (!safeView) {
            return;
        }
        QCoro::connect(safeView->loadPreparedLibrary(), safeView.data(), []() {});
    });
...
QWidget *HomeView::createPersonView(const QString &personId, const QString &personName)
{
    ...
    view->preparePerson(personId, personName);
    const QPointer<LibraryView> safeView(view);
    scheduleDeferredInitialLoad(view, [safeView]() {
        if (!safeView) {
            return;
        }
        QCoro::connect(safeView->loadPreparedPerson(), safeView.data(), []() {});
    });
...
QWidget *HomeView::createSearchView(const QString &query)
{
    ...
    view->prepareSearch(query);
    const QPointer<SearchView> safeView(view);
    scheduleDeferredInitialLoad(view, [safeView]() {
        if (!safeView) {
            return;
        }
        QCoro::connect(safeView->loadPreparedSearch(), safeView.data(), []() {});
    });
...
QWidget *HomeView::createFilteredView(const QString &filterType, const QString &filterValue)
{
    ...
    view->prepareFiltered(filterType, filterValue);
    const QPointer<LibraryView> safeView(view);
    scheduleDeferredInitialLoad(view, [safeView]() {
        if (!safeView) {
            return;
        }
        QCoro::connect(safeView->loadPreparedFiltered(), safeView.data(), []() {});
    });
...
QWidget *HomeView::createSeasonView(const QString &seriesId, const QString &seasonId, const QString &seasonName)
{
    ...
    view->prepareSeason(seriesId, seasonId, seasonName);
    const QPointer<SeasonView> safeView(view);
    scheduleDeferredInitialLoad(view, [safeView]() {
        if (!safeView) {
            return;
        }
        QCoro::connect(safeView->loadPreparedSeason(), safeView.data(), []() {});
    });
...
void HomeView::setupSidebar()
{
    ...
    if (!m_sidebarPinned)
    {
        m_sidebarShadow = new QGraphicsDropShadowEffect(this);
        m_sidebarShadow->setBlurRadius(25);
        m_sidebarShadow->setColor(QColor(0, 0, 0, 30));
        m_sidebarShadow->setOffset(m_sidebarOnRight ? -4 : 4, 0);
        m_sidebar->setGraphicsEffect(m_sidebarShadow);
    }
    ...
    m_sidebarLibraryScrollController->setDuration(XplayerUi::kScrollAnimationMs);
    ...
    m_sidebarAnim->setDuration(
        XplayerUi::durationMs(XplayerUi::MotionDuration::Panel));
    m_sidebarAnim->setEasingCurve(
        XplayerUi::easingCurve(XplayerUi::MotionCurve::Move));
    connect(m_sidebarAnim, &QPropertyAnimation::finished, this,
            [this]() { setSidebarAnimationActive(false); });
    ...
    m_sidebarAutoHideTimer->setInterval(XplayerUi::kSidebarAutoHideMs);
...
void HomeView::pushView(QWidget *view)
{
    ...
    const QString currentRouteKey = routeKeyForWidget(current);
    const QString nextRouteKey = routeKeyForWidget(view);
    if (!currentRouteKey.isEmpty() && currentRouteKey == nextRouteKey)
    {
    ...
    if (current)
    {
        ...
        if (m_navStack.isEmpty() || routeKeyForInfo(m_navStack.top()) != routeKeyForInfo(info))
        {
    ...
    for (int i = m_navStack.size() - 1; i >= 0; --i)
    {
        if (m_navStack[i].isDynamic && m_navStack[i].widget)
        {
            ...
            {
                QWidget *staleWidget = m_navStack[i].widget.data();
                m_navStack[i].widget.clear();
                if (staleWidget && staleWidget != current && staleWidget != view)
                {
                    m_contentSwitcher->disposeWidgetWhenSafe(staleWidget);
                }
            }
        }
    }
...
void HomeView::scheduleDeferredInitialLoad(QWidget *view,
                                           std::function<void()> startLoad)
{
    if (!view || !m_contentSwitcher || !startLoad)
    {
        return;
    }

    view->setProperty("initialLoadStarted", false);
    const QPointer<QWidget> safeView(view);
    const QPointer<SlidingStackedWidget> safeSwitcher(m_contentSwitcher);
    const auto runLoad = [safeView, safeSwitcher, startLoad]() mutable
    {
        if (!safeView || !safeSwitcher ||
            safeSwitcher->currentWidget() != safeView ||
            safeView->property("initialLoadStarted").toBool())
        {
            return;
        }

        safeView->setProperty("initialLoadStarted", true);
        startLoad();
    };

    connect(m_contentSwitcher, &SlidingStackedWidget::animationFinished,
            view, runLoad);
    QTimer::singleShot(XplayerUi::kDeferredInitialLoadDelayMs, view, runLoad);
}

void HomeView::navigateBack()
{
    QWidget *current = m_contentSwitcher->currentWidget();
    QPointer<BaseView> deferredTeardownView;
    ...
    if (auto *baseView = qobject_cast<BaseView *>(current))
    {
        ...
        const bool deferTeardownForTransition =
            current->property("isImmersive").toBool() ||
            current->property("deferTeardownForTransition").toBool();
        if (deferTeardownForTransition)
        {
            deferredTeardownView = baseView;
        }
        else
        {
    ...
    if (!m_navStack.isEmpty())
    {
        ...
        if (deferredTeardownView)
        {
            QTimer::singleShot(XplayerUi::kDetailHeavyLoadDelayMs, deferredTeardownView,
                               [deferredTeardownView]()
                               {
                                   if (deferredTeardownView)
                                   {
                                       deferredTeardownView->prepareForStackLeave();
                                   }
                               });
        }
        Q_EMIT canNavigateBackChanged(!m_navStack.isEmpty());
...
void HomeView::goFav()
{
    if (m_contentSwitcher->currentWidget() == m_favoritesView)
    {
        ...
        m_favoritesView->requestFavoritesRefresh(true);
        return;
    }
    m_libraryList->clearSelection();
...
void HomeView::showSidebar()
{
    ...
    if (reduceAnimations)
    {
        setSidebarAnimationActive(false);
        const QPoint target = m_sidebarOnRight
                                  ? QPoint(width() - m_sidebar->width(), 0)
                                  : QPoint(0, 0);
        XplayerUi::moveIfChanged(m_sidebar, target);
        m_sidebarAutoHideTimer->start(); 
        return;
    }
    ...
    const QPoint target = m_sidebarOnRight
                              ? QPoint(width() - m_sidebar->width(), 0)
                              : QPoint(0, 0);
    if (m_sidebar->pos() == target ||
        (m_sidebarAnim->state() == QAbstractAnimation::Running &&
         m_sidebarAnim->endValue().toPoint() == target))
    {
        m_sidebarAutoHideTimer->start();
        return;
    }
    ...
    if (m_sidebarAnim->state() == QAbstractAnimation::Running)
        m_sidebarAnim->stop();
    setSidebarAnimationActive(true);
...
void HomeView::hideSidebar()
{
    ...
    if (reduceAnimations)
    {
        ...
        const QPoint target =
            m_sidebarOnRight
                ? QPoint(width() + kSidebarHiddenOffset, 0)
                : QPoint(-m_sidebar->width() - kSidebarHiddenOffset, 0);
        XplayerUi::moveIfChanged(m_sidebar, target);
        return;
    }
...
void HomeView::setSidebarAnimationActive(bool active)
{
    if (!m_sidebar || !m_sidebarShadow ||
        m_sidebar->graphicsEffect() != m_sidebarShadow)
    {
        return;
    }

    m_sidebarShadow->setEnabled(!active);
}
...
void HomeView::applySidebarPinned(bool pinned)
{
    ...
    else
    {
        ...
        m_sidebarShadow->setOffset(m_sidebarOnRight ? -4 : 4, 0);
        m_sidebarShadow->setEnabled(true);
        m_sidebar->setGraphicsEffect(m_sidebarShadow);
