...
#include "../../utils/playerinpututils.h"
#include "../../utils/playerwindowgeometryutils.h"
...
PlayerView::PlayerView(XplayerCore *core, QWidget *parent)
    : BaseView(core, parent), m_isPlaying(false), m_currentPosition(0.0), m_totalDuration(0.0), m_activePopup(nullptr)
{
    ...
    connect(m_bufferTimer, &QTimer::timeout, this,
            [this]()
            {
                ...
                {
                    double bufferedPos =
                        m_hasBufferedPosition ? m_bufferedPosition : m_currentPosition;
    ...
    m_mousePollTimer->setInterval(XplayerUi::kPointerPollMs);
...
void PlayerView::showHudMediaSwitcher()
{
    ...
    m_mediaSwitchDrawer->setLoadingState(
        m_isSeriesMode ? (m_seriesName.isEmpty()
                              ? PlayerMediaSwitcherTextUtils::seriesFallbackTitle()
                              : m_seriesName)
                       : PlayerMediaSwitcherTextUtils::movieModeTitle(),
        PlayerMediaSwitcherTextUtils::loadingMessage(), m_isSeriesMode);
    m_mediaSwitchDrawer->show();
    updateMediaSwitcherButton();
    updateOverlayLayout();
    showControls();
    QCoro::connect(ensureMediaSwitcherDataLoaded(), this, []() {});
}
...
void PlayerView::updatePowerInhibition()
{
    ...
    if (shouldHold)
    {
        m_powerInhibitionHeld = PowerInhibitUtils::acquirePlaybackInhibition(QStringLiteral("Xplayer video playback"));
    }
    else
    {
...
QCoro::Task<void> PlayerView::autoPlayNextMediaIfEnabled()
{
    ...
    qInfo().noquote() << "[PlayerView] Continuous playback switching to next item"
...
void PlayerView::refreshForScreenChange()
{
    if (m_isViewTearingDown)
    {
        return;
    ...
    applyResponsiveLayout();
    updateOverlayLayout();
    updateWindowMaximizeButtonState();
}
...
void PlayerView::applyResponsiveLayout()
{
    const qreal scale = XplayerResponsiveUtils::scaleForViewport(size());
    m_bottomHudBaseHeight = XplayerResponsiveUtils::scaled(110, scale, 88, 160);

    if (m_topHUD) {
        m_topHUD->setFixedHeight(XplayerResponsiveUtils::scaled(32, scale, 30, 48));
    }

    auto applyTopButton = [scale](QPushButton *button, const QSize &baseIcon) {
        if (!button) {
            return;
        }
        const QSize iconSize(
            XplayerResponsiveUtils::scaled(baseIcon.width(), scale, baseIcon.width(), qRound(baseIcon.width() * 1.45)),
            XplayerResponsiveUtils::scaled(baseIcon.height(), scale, baseIcon.height(), qRound(baseIcon.height() * 1.45)));
        button->setIconSize(iconSize);
        button->setFixedSize(
            XplayerResponsiveUtils::scaled(38, scale, 34, 56),
            XplayerResponsiveUtils::scaled(28, scale, 28, 42));
    };
    applyTopButton(m_backBtn, QSize(16, 16));
    applyTopButton(m_minBtn, QSize(12, 12));
    applyTopButton(m_maxBtn, QSize(12, 12));
    applyTopButton(m_closeBtn, QSize(12, 12));

    if (auto *bottomLayout = qobject_cast<QVBoxLayout *>(m_bottomHUD ? m_bottomHUD->layout() : nullptr)) {
        bottomLayout->setContentsMargins(
            XplayerResponsiveUtils::scaled(32, scale, 22, 48),
            XplayerResponsiveUtils::scaled(10, scale, 8, 16),
            XplayerResponsiveUtils::scaled(32, scale, 22, 48),
            XplayerResponsiveUtils::scaled(16, scale, 12, 24));
        bottomLayout->setSpacing(XplayerResponsiveUtils::scaled(8, scale, 6, 14));
        if (auto *progressLayout = bottomLayout->itemAt(1) ? bottomLayout->itemAt(1)->layout() : nullptr) {
            progressLayout->setSpacing(XplayerResponsiveUtils::scaled(16, scale, 10, 24));
        }
        if (auto *controlLayout = bottomLayout->itemAt(2) ? bottomLayout->itemAt(2)->layout() : nullptr) {
            controlLayout->setSpacing(XplayerResponsiveUtils::scaled(12, scale, 8, 18));
        }
    }

    auto applyHudButton = [scale](QPushButton *button, const QSize &baseIcon) {
        if (!button) {
            return;
        }
        const QSize iconSize(
        ...
        button->setIconSize(iconSize);
        button->setFixedSize(
            iconSize.width() + XplayerResponsiveUtils::scaled(16, scale, 12, 24),
            iconSize.height() + XplayerResponsiveUtils::scaled(16, scale, 12, 24));
    };
    applyHudButton(m_prevMediaBtn, QSize(24, 24));
    applyHudButton(m_rewindBtn, QSize(24, 24));
    applyHudButton(m_playPauseBtn, QSize(36, 36));
    applyHudButton(m_forwardBtn, QSize(24, 24));
    applyHudButton(m_nextMediaBtn, QSize(24, 24));
    applyHudButton(m_volumeBtn, QSize(24, 24));
    applyHudButton(m_mediaSwitchBtn, QSize(24, 24));
    applyHudButton(m_audioBtn, QSize(24, 24));
    applyHudButton(m_subtitleBtn, QSize(24, 24));
    applyHudButton(m_danmakuBtn, QSize(24, 24));
    applyHudButton(m_settingsBtn, QSize(24, 24));
    applyHudButton(m_scaleBtn, QSize(24, 24));
    applyHudButton(m_bottomMaximizeBtn, QSize(24, 24));

    if (m_volumeSlider) {
        m_volumeSlider->setFixedWidth(XplayerResponsiveUtils::scaled(80, scale, 66, 118));
    }
    if (m_speedBtn) {
        m_speedBtn->setFixedHeight(XplayerResponsiveUtils::scaled(40, scale, 34, 58));
    }
    if (m_rightTrigger) {
        m_rightTrigger->setFixedWidth(XplayerResponsiveUtils::scaled(15, scale, 12, 22));
    }
    if (m_rightSidebar) {
        m_rightSidebar->setFixedWidth(XplayerResponsiveUtils::scaled(220, scale, 184, 320));
        if (auto *layout = qobject_cast<QVBoxLayout *>(m_rightSidebar->layout())) {
            layout->setContentsMargins(0,
                                        XplayerResponsiveUtils::scaled(15, scale, 10, 24),
                                        0,
                                        XplayerResponsiveUtils::scaled(15, scale, 10, 24));
            layout->setSpacing(XplayerResponsiveUtils::scaled(10, scale, 8, 16));
        }
    }
...
void PlayerView::setupUi()
{
    ...
    m_maxBtn->setIcon(QIcon(PlaybackWindowModeUtils::topbarMaximizeIconPath(false)));
    m_maxBtn->setIconSize(QSize(12, 12));
    m_maxBtn->setFixedSize(38, 28);
    m_maxBtn->setFocusPolicy(Qt::NoFocus);
    m_maxBtn->setToolTip(PlaybackWindowModeUtils::topbarMaximizeTooltipText(false));
    ...
    connect(m_maxBtn, &QPushButton::clicked, this, &PlayerView::toggleWindowMaximized);
    ...
    connect(m_mediaSwitchDrawer, &PlayerMediaSwitcherPanel::playRequested, this,
            [this](QString mediaId, QString title, long long startPositionTicks)
            {
                QCoro::connect(
                    switchFromMediaSwitcher(mediaId, title, startPositionTicks),
                    this, []() {});
            });
    ...
    m_bottomMaximizeBtn = createHudButton(PlaybackWindowModeUtils::maximizeIconPath(false));
    m_bottomMaximizeBtn->setToolTip(PlaybackWindowModeUtils::maximizeTooltipText(false));
    ...
    controlLayout->addWidget(m_bottomMaximizeBtn);
    ...
    connect(m_prevMediaBtn, &QPushButton::clicked, this,
            [this]()
            {
                ...
                QCoro::connect(
                    switchFromMediaSwitcher(targetId, targetTitle, startTicks),
                    this, []() {});
            });
    ...
    connect(m_bottomMaximizeBtn, &QPushButton::clicked, this, &PlayerView::toggleWindowMaximized); 
    updateWindowMaximizeButtonState();
    ...
    connect(m_mpvWidget->controller(), &MpvController::endOfFile, this,
            [this](const QString &reason)
            {
                ...
                {
                    QCoro::connect(autoPlayNextMediaIfEnabled(), this, []() {});
                }
                else
                {
                    const QString message =
                        PlaybackEngineProfile::playerFacingFailureMessage(reason);
                    if (!message.isEmpty())
                    {
                        showControls();
                        showToast(message);
    ...
    connect(m_mpvWidget, &MpvWidget::errorOccurred, this,
            [this](const QString &message)
            {
                ...
                updateLoadingState();
                showControls();
                showToast(message.trimmed().isEmpty()
                              ? PlaybackEngineProfile::playerFacingFailureMessage(QStringLiteral("error"))
                              : message);
            });
...
void PlayerView::updateLoadingState()
{
    ...
    context.hasMedia = !m_currentMediaId.isEmpty();
    context.isOpening = m_isOpeningMedia;
    context.isBuffering = m_isBuffering && !m_isOpeningMedia;
    context.isSeeking = m_isSeeking;
    context.isTearingDown = m_isViewTearingDown;

    const PlaybackLoadingStateUtils::State state =
        PlaybackLoadingStateUtils::resolveState(context);

    if (state == PlaybackLoadingStateUtils::State::Hidden)
    {
        if (m_isViewTearingDown)
        {
    ...
    m_loadingOverlay->setMessage(PlaybackLoadingStateUtils::displayText(state));
...
void PlayerView::closeActivePopup()
{
    if (!m_activePopup)
    {
        m_activePopupButton.clear();
        return;
    }

    QWidget *popup = m_activePopup;
...
void PlayerView::showCenteredPopup(QWidget *popup, QPushButton *btn)
{
    ...
    const auto action = PlayerPopupStateUtils::actionForPopupRequest(
        m_activePopup != nullptr, m_activePopupButton && m_activePopupButton == btn);
    if (action == PlayerPopupStateUtils::PopupRequestAction::CloseCurrent)
    {
        if (popup)
        {
            popup->deleteLater();
    ...
    if (action == PlayerPopupStateUtils::PopupRequestAction::Replace)
    {
        closeActivePopup();
    }

    m_activePopup = popup;
    m_activePopupButton = btn;
    ...
    const QPoint finalPos(mx, my);
    popup->move(finalPos + QPoint(0, PlayerPopupAnimationUtils::openOffsetY()));
    popup->show();
    popup->raise();
    ...
    XplayerUi::animateProperty(
        ...
        XplayerUi::MotionSpec{XplayerUi::MotionDuration::Quick,
                              XplayerUi::MotionCurve::Enter,
                              PlayerPopupAnimationUtils::openDurationMs()});
...
void PlayerView::setupRightSidebar()
{
    ...
    connect(m_resumeList, &QListWidget::itemDoubleClicked, this,
            [this](QListWidgetItem *item)
            {
                ...
                QCoro::connect(
                    switchFromMediaSwitcher(item->data(Qt::UserRole).toString(),
    ...
    m_rightSidebarAnim->setDuration(
        XplayerUi::durationMs(XplayerUi::MotionDuration::Panel));
    m_rightSidebarAnim->setEasingCurve(
...
QCoro::Task<void> PlayerView::showRightSidebar()
{
    ...
    else
    {
        ...
        m_rightSidebar->show();
        const QPoint target(width() - m_rightSidebar->width(), sidebarY);
        if (m_rightSidebar->pos() != target)
        {
            m_rightSidebarAnim->setStartValue(m_rightSidebar->pos());
            m_rightSidebarAnim->setEndValue(target);
...
void PlayerView::hideRightSidebar(bool immediate)
{
    ...
    if (m_rightSidebar->pos() != hiddenPos)
    {
        m_rightSidebarAnim->setStartValue(m_rightSidebar->pos());
...
void PlayerView::stopAndReport()
{
    ...
    ++m_progressGeneration;
    m_progressPolicy.reset();
    updatePowerInhibition();
    ...
    if (!m_currentMediaId.isEmpty() && m_core && m_core->mediaService())
    {
        ...
        QCoro::connect(taskRoutine(service, mediaId, sourceId, currentTicks, sessionId),
                       m_core, []() {});
    }
...
void PlayerView::updateOverlayLayout()
{
    ...
    const int targetBottomHudHeight =
        m_bottomHudBaseHeight +
        (drawerHeight > 0 ? drawerHeight + XplayerResponsiveUtils::scaled(8, scale, 6, 14) : 0);
    ...
    if (m_logoLabel)
    {
        m_logoLabel->move(XplayerResponsiveUtils::scaled(24, scale, 16, 36),
                          m_topHUD->height());
    }

    if (m_networkSpeedLabel)
    {
        const int speedWidth = XplayerResponsiveUtils::scaled(200, scale, 160, 290);
        const int speedHeight = XplayerResponsiveUtils::scaled(30, scale, 26, 44);
        m_networkSpeedLabel->setGeometry(
            width() - speedWidth - XplayerResponsiveUtils::scaled(10, scale, 8, 16),
            m_topHUD->height(), speedWidth, speedHeight);
    }

    if (m_statisticsOverlay)
    {
        int statisticsY =
            m_topHUD->height() + XplayerResponsiveUtils::scaled(10, scale, 8, 16);
        if (m_logoLabel && m_logoLabel->isVisible())
        {
            statisticsY =
                qMax(statisticsY, m_logoLabel->y() + m_logoLabel->height() +
                                      XplayerResponsiveUtils::scaled(8, scale, 6, 14));
        }

        const int statisticsWidth =
            qMax(XplayerResponsiveUtils::scaled(280, scale, 220, 406),
                 qMin(width() - XplayerResponsiveUtils::scaled(24, scale, 18, 36),
                      XplayerResponsiveUtils::scaled(620, scale, 460, 899)));
        m_statisticsOverlay->setFixedWidth(statisticsWidth);
        m_statisticsOverlay->setFixedHeight(m_statisticsOverlay->sizeHint().height());
        m_statisticsOverlay->move(XplayerResponsiveUtils::scaled(12, scale, 10, 18),
                                  statisticsY);
    }
    ...
    const int triggerWidth = m_rightTrigger ? m_rightTrigger->width()
                                            : XplayerResponsiveUtils::scaled(15, scale, 12, 22);
    m_rightTrigger->setGeometry(width() - triggerWidth, sidebarY, triggerWidth, sidebarH);

    if (!m_isRightSidebarVisible)
    {
        m_rightSidebar->setGeometry(width() + XplayerResponsiveUtils::scaled(30, scale, 22, 44),
                                    sidebarY, m_rightSidebar->width(), sidebarH);
    }
    else
    {
...
bool PlayerView::eventFilter(QObject *watched, QEvent *event)
{
    ...
    {
        QCoro::connect(showRightSidebar(), this, []() {});
        return true;
    }
    ...
    if (event->type() == QEvent::Wheel)
    {
            ...
            watched == this)
        {
            ...
            const int volumeDelta = PlayerInputUtils::consumeWheelVolumeDelta(
                we->angleDelta().y(), we->pixelDelta().y(),
                m_wheelAngleRemainder, m_wheelPixelRemainder);
            if (volumeDelta != 0)
            {
                changeVolume(volumeDelta);
                return true;
            }
            return !we->angleDelta().isNull() || !we->pixelDelta().isNull();
        }
    }
...
void PlayerView::handlePointerActivity(const QPoint &globalPos)
{
    ...
    const PlayerHudVisibilityUtils::AutoHideContext context =
        buildHudAutoHideContext(localPos);

    if (PlayerHudVisibilityUtils::shouldUseAutoHideTimer(context))
    {
...
PlayerHudVisibilityUtils::AutoHideContext PlayerView::buildHudAutoHideContext(const QPoint &localPos) const
{
    PlayerHudVisibilityUtils::AutoHideContext context;
    context.isPlaying = m_isPlaying;
    context.isLoading = m_isBuffering || m_isSeeking;
    context.hasActiveDialog = m_activePlayerDialog && m_activePlayerDialog->isVisible();
    context.isAppActive = window() && window()->isActiveWindow();
    context.pointerInsidePlayer = rect().contains(localPos);
    context.pointerOnChrome =
        (m_topHUD && m_topHUD->geometry().contains(localPos)) ||
        (m_bottomHUD && m_bottomHUD->geometry().contains(localPos));
    context.pointerOnPopup = m_activePopup && m_activePopup->geometry().contains(localPos);
    context.rightSidebarVisible = m_isRightSidebarVisible;
    context.pointerOnRightSidebar =
        m_rightSidebar && m_rightSidebar->geometry().contains(localPos);
    context.mediaSwitcherVisible = m_mediaSwitchDrawer && m_mediaSwitchDrawer->isVisible();
    return context;
}
...
void PlayerView::showControls()
{
    ...
    const PlayerHudVisibilityUtils::AutoHideContext context =
        buildHudAutoHideContext(mapFromGlobal(QCursor::pos()));
    if (PlayerHudVisibilityUtils::shouldUseAutoHideTimer(context))
    {
        m_hideTimer->start(kHudAutoHideDelayMs);
    }
...
void PlayerView::hideControls()
{
    ...
    const PlayerHudVisibilityUtils::AutoHideDecision decision =
        PlayerHudVisibilityUtils::autoHideDecision(buildHudAutoHideContext(localPos));
    if (decision == PlayerHudVisibilityUtils::AutoHideDecision::KeepVisibleAndRetry)
    {
        m_hideTimer->start(850);
        setCursorHidden(false);
        return;
    }
    if (decision == PlayerHudVisibilityUtils::AutoHideDecision::KeepVisibleNoRetry)
    {
        m_hideTimer->stop();
        setCursorHidden(false);
        return;
...
void PlayerView::onMpvPropertyChanged(const QString &property, const QVariant &value)
{
    ...
    {
        const QVariantMap cacheState = value.toMap();
        const QVariantList ranges =
            cacheState.value(QStringLiteral("seekable-ranges")).toList();
        if (!ranges.isEmpty())
        {
            const QVariantMap lastRange = ranges.constLast().toMap();
            const double end = lastRange.value(QStringLiteral("end")).toDouble();
            if (std::isfinite(end) && end >= 0.0)
            {
                m_bufferedPosition = end;
                m_hasBufferedPosition = true;
            }
        }
    }
    else if (property == "demuxer-cache-duration")
    {
        const double cacheDuration = value.toDouble();
        if (std::isfinite(cacheDuration) && cacheDuration >= 0.0)
        {
            m_bufferedPosition = m_currentPosition + cacheDuration;
            m_hasBufferedPosition = true;
        }
    }
...
void PlayerView::changeVolume(int delta, bool silent)
{
    ...
    ConfigStore::instance()->setDeferred(ConfigKeys::PlayerVolumeLevel,
                                         m_currentVolume);
}
...
void PlayerView::onVolumeSliderMoved(int value)
{
    ...
    ConfigStore::instance()->setDeferred(ConfigKeys::PlayerVolumeLevel,
                                         m_currentVolume);
    ConfigStore::instance()->setDeferred(ConfigKeys::PlayerVolumeMuted,
                                         m_isMuted);
}
...
void PlayerView::showSettingsMenu()
{
    ...
    panel->addItem(PlayerControlTextUtils::networkSpeedLabel(), "network_speed", m_showNetworkSpeed);
    panel->addItem(PlayerControlTextUtils::statisticsLabel(), "statistics", m_showStatisticsOverlay);
    panel->addItem(PlayerControlTextUtils::subtitleSettingsLabel(), "subtitle_settings", false);
    if (showDanmakuControls)
    {
        panel->addItem(PlayerControlTextUtils::danmakuSettingsLabel(), "danmaku_settings", false);
    }
    ...
    panel->addItem(PlayerControlTextUtils::strmDirectPlayLabel(), "strm_direct", strmDirect);
    ...
    connect(panel, &ModernScrollPanel::itemTriggered, this,
            [this](const QVariant &data, const QString &text)
            {
                ...
                if (action == "network_speed")
                {
                    ...
                    showToast(PlayerControlTextUtils::networkSpeedToast(m_showNetworkSpeed));
                }
                ...
                else if (action == "strm_direct")
                {
                    ...
                    showToast(PlayerControlTextUtils::strmDirectToast(newStrm));
...
void PlayerView::showSpeedMenu()
{
    ...
    connect(panel, &ModernScrollPanel::itemTriggered, this,
            [this](const QVariant &data, const QString &text)
            {
                ...
                showToast(PlayerControlTextUtils::speedToast(
                    PlayerLongPressHandler::formatSpeedText(s).chopped(1)));
...
void PlayerView::showAudioMenu()
{
    ...
    for (const QVariant &v : tracks)
    {
        ...
        if (map["type"].toString() == "audio")
        {
            ...
            QString text = PlayerTrackOptionTextUtils::trackLabel(
                PlayerTrackOptionTextUtils::TrackKind::Audio, id, title, lang);
            panel->addItem(text, id, selected);
        }
    }

    panel->addItem(PlayerTrackOptionTextUtils::disableLabel(
    ...
    connect(panel, &ModernScrollPanel::itemTriggered, this,
            [this](const QVariant &data, const QString &text)
            {
                ...
                if (valStr == "no")
                {
                    ...
                    showToast(PlayerTrackOptionTextUtils::disabledToast(
                        PlayerTrackOptionTextUtils::TrackKind::Audio));
                }
                else
                {
                    ...
                    showToast(PlayerTrackOptionTextUtils::selectedToast(
                        PlayerTrackOptionTextUtils::TrackKind::Audio, text));
                }
...
void PlayerView::showSubtitleMenu()
{
    ...
    for (const QVariantMap &map : tracks)
    {
        ...
        QString text = PlayerTrackOptionTextUtils::trackLabel(
            PlayerTrackOptionTextUtils::TrackKind::Subtitle, id, title, lang);
        panel->addItem(text, id, selected);
    }
    ...
    panel->addItem(PlayerControlTextUtils::subtitleSettingsLabel(), QStringLiteral("settings"), false);
    ...
    connect(panel, &ModernScrollPanel::itemTriggered, this,
            [this](const QVariant &data, const QString &text)
            {
                ...
                showToast(action == "no"
                              ? PlayerTrackOptionTextUtils::disabledToast(
                                    PlayerTrackOptionTextUtils::TrackKind::Subtitle)
                              : PlayerTrackOptionTextUtils::selectedToast(
                                    PlayerTrackOptionTextUtils::TrackKind::Subtitle, text));
                dismissPopup();
            });
...
void PlayerView::showToast(const QString &msg)
{
    ...
    m_toastLabel->show();
    m_toastTimer->start(XplayerUi::kToastHoldMs);
}
...
void PlayerView::updateWindowMaximizeButtonState()
{
    if (!window())
    {
        return;
    ...
    const bool isMaximized = window()->isFullScreen() || window()->isMaximized();
    if (m_bottomMaximizeBtn)
    {
        m_bottomMaximizeBtn->setIcon(QIcon(PlaybackWindowModeUtils::maximizeIconPath(isMaximized)));
        m_bottomMaximizeBtn->setToolTip(PlaybackWindowModeUtils::maximizeTooltipText(isMaximized));
        m_bottomMaximizeBtn->setProperty("isMaximized", isMaximized);
        m_bottomMaximizeBtn->style()->unpolish(m_bottomMaximizeBtn);
        m_bottomMaximizeBtn->style()->polish(m_bottomMaximizeBtn);
        m_bottomMaximizeBtn->update();
    }
    if (m_maxBtn)
    {
        m_maxBtn->setIcon(QIcon(PlaybackWindowModeUtils::topbarMaximizeIconPath(isMaximized)));
        m_maxBtn->setToolTip(PlaybackWindowModeUtils::topbarMaximizeTooltipText(isMaximized));
        m_maxBtn->setProperty("isMax", isMaximized);
        m_maxBtn->style()->unpolish(m_maxBtn);
        m_maxBtn->style()->polish(m_maxBtn);
        m_maxBtn->update();
    }
}

void PlayerView::toggleWindowMaximized()
{
    if (!window())
    {
        return;
    ...
    const bool isFullscreen = topLevel->isFullScreen();
    if (isFullscreen)
    {
        topLevel->showNormal();
    ...
    {
        m_restoreMaximizedAfterFullscreen = topLevel->isMaximized();
        topLevel->showFullScreen();
        topLevel->raise();
        topLevel->activateWindow();
    }
    QTimer::singleShot(0, this, &PlayerView::updateWindowMaximizeButtonState);
}


QCoro::Task<void> PlayerView::executeFetchLogo(QPointer<PlayerView> safeThis, XplayerCore *core, QString mediaId)
{
    try
    {
                           ...
                           long long startPositionTicks, const QVariant &sourceInfoVar)
{
    ...
    m_totalDuration = 0.0;
    m_bufferedPosition = m_currentPosition;
    m_hasBufferedPosition = false;
    m_lastPresentedPositionSecond = qMax(0, qRound(m_currentPosition));
    ...
    if (resolvedItem.type == "Episode" && !resolvedItem.seriesId.isEmpty())
    {
        ...
        updateMediaSwitcherButton();
        QCoro::connect(requestIntroDBSegments(), this, []() {});
    ...
    else
    {
        ...
        auto detectSeriesMode = [](QPointer<PlayerView> safeThis, XplayerCore *core, QString mId) -> QCoro::Task<void>
        {
            try
            {
                ...
                if (detail.type == "Episode" && !detail.seriesId.isEmpty())
                {
                    ...
                    QCoro::connect(safeThis->requestIntroDBSegments(),
                                   safeThis.data(), []() {});
                    
                    QCoro::connect(safeThis->ensureMediaSwitcherDataLoaded(),
                                   safeThis.data(), []() {});
                    ...
                    {
                        QCoro::connect(safeThis->showRightSidebar(),
                                       safeThis.data(), []() {});
                    }
                }
            }
    ...
    QCoro::connect(executeFetchLogo(QPointer<PlayerView>(this), m_core, mediaId),
                   this, []() {});
    ...
    else if (!mediaId.isEmpty())
    {
        ...
        QCoro::connect(resolveDanmakuPlaybackContext(
                           QPointer<PlayerView>(this), QPointer<XplayerCore>(m_core),
    ...
    QCoro::connect(
        startSessionTask(QPointer<PlayerView>(this), m_core->mediaService(),
...
void PlayerView::reportProgressToServer()
{
    ...
    long long currentTicks = static_cast<long long>(m_currentPosition * 10000000.0);
    queuePlaybackProgressReport(currentTicks, !m_isPlaying, false);
...
void PlayerView::queuePlaybackProgressReport(long long positionTicks,
                                              bool isPaused,
                                              bool force)
{
    if (m_isViewTearingDown || m_hasReportedStop ||
        m_currentMediaId.isEmpty() || m_currentPlaySessionId.isEmpty() ||
        !m_core || !m_core->mediaService()) {
        return;
    }

    const auto dispatch = m_progressPolicy.enqueue(
        PlaybackProgressRequest{positionTicks, isPaused, force});
    if (!dispatch.has_value()) {
        return;
    }

    const QString mediaId = m_currentMediaId;
    const QString mediaSourceId = m_currentMediaSourceId;
    const QString playSessionId = m_currentPlaySessionId;
    const quint64 generation = m_progressGeneration;
    QCoro::connect(
        m_core->mediaService()->reportPlaybackProgress(
            mediaId, mediaSourceId, dispatch->positionTicks,
            dispatch->isPaused, playSessionId),
        m_core, [this, generation]() {
            if (generation != m_progressGeneration) {
                return;
            }

            const auto next = m_progressPolicy.complete();
            if (next.has_value()) {
                queuePlaybackProgressReport(
                    next->positionTicks, next->isPaused, next->force);
            }
        });
}
...
void PlayerView::onBackClicked()
{
    ...
    if (win)
    {
        ...
        QTimer::singleShot(XplayerUi::kPanelAnimationMs +
                               XplayerUi::kFrameIntervalMs,
                           safeWin.data(),
...
void PlayerView::onPositionChanged(double position)
{
    ...
    const int presentedSecond = qMax(0, qRound(position));

    checkAndSkipSegment(position);

    if (presentedSecond != m_lastPresentedPositionSecond)
    {
        m_lastPresentedPositionSecond = presentedSecond;
        if (!m_progressSlider->isSliderDown())
        {
            m_progressSlider->blockSignals(true);
            if (m_progressSlider->value() != presentedSecond)
            {
                m_progressSlider->setValue(presentedSecond);
            }
            m_progressSlider->blockSignals(false);
        }

        const QString currentTimeText = formatTime(position, m_totalDuration);
        if (m_currentTimeLabel->text() != currentTimeText)
        {
    ...
    if (m_osdLayer && m_osdLayer->isSeekLineVisible())
    {
        if (m_osdSeekPreviewPosition < 0.0 || std::abs(position - m_osdSeekPreviewPosition) <= 1.0)
        {
            ...
            m_osdLayer->updateSeekPosition(static_cast<int>(position),
                                           formatTime(position, m_totalDuration));
        }
    }
}
...
void PlayerView::onDurationChanged(double duration)
{
    ...
    const int durationSeconds = static_cast<int>(duration);
    if (m_progressSlider->maximum() != durationSeconds)
    {
        m_progressSlider->setMaximum(durationSeconds);
    }
    const QString durationText = formatTime(duration, duration);
    if (m_totalTimeLabel->text() != durationText)
    {
        m_totalTimeLabel->setText(durationText);
    }
    ...
    if (!m_hasSetVideoSize && duration > 0)
    {
        ...
        if (win && !m_wasMaximized && !win->isMaximized() && !qobject_cast<QMainWindow *>(win))
        {
            ...
            const QRect currentGeo = win->geometry();
            QRect targetGeometry(currentGeo.center().x() - targetWinW / 2,
                                 currentGeo.center().y() - targetWinH / 2,
                                 targetWinW, targetWinH);
            QScreen *targetScreen = win->screen();
            if (!targetScreen && win->windowHandle())
                targetScreen = win->windowHandle()->screen();
            if (!targetScreen)
                targetScreen = QGuiApplication::primaryScreen();
            if (targetScreen)
            {
                targetGeometry =
                    PlayerWindowGeometryUtils::boundedVideoGeometry(
                        targetGeometry, targetScreen->availableGeometry(),
                        aspect);
            }
            win->setGeometry(targetGeometry);
        }
...
void PlayerView::onPlaybackStateChanged(bool isPaused)
{
    ...
    {
        long long currentTicks = static_cast<long long>(m_currentPosition * 10000000.0);
        queuePlaybackProgressReport(currentTicks, isPaused, true);
    }
}
...
void PlayerView::onSliderMoved(int value)
{
    ...
    if (!m_hasReportedStop && !m_currentMediaId.isEmpty() && !m_currentPlaySessionId.isEmpty())
    {
        ...
        queuePlaybackProgressReport(currentTicks, !m_isPlaying, true);
    }
