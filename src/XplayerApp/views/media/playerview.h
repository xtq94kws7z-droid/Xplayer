...
class PlayerView : public BaseView {
    Q_OBJECT
public:
    explicit PlayerView(XplayerCore *core, QWidget *parent = nullptr);
    ~PlayerView() override;
...
private slots:
    ...
    void queuePlaybackProgressReport(long long positionTicks,
                                     bool isPaused,
                                     bool force);
    void onBackClicked();
...
private:
    ...
    void updateWindowMaximizeButtonState();
    PlayerHudVisibilityUtils::AutoHideContext buildHudAutoHideContext(const QPoint &localPos) const;
    ...
    QPointer<QPushButton> m_activePopupButton;
    QPointer<PlayerOverlayDialog> m_activePlayerDialog;

    
    static QCoro::Task<void> executeFetchLogo(QPointer<PlayerView> safeThis, XplayerCore* core, QString mediaId);
    ...
    QPushButton *m_bottomMaximizeBtn = nullptr;
    ...
    PlaybackProgressPolicy m_progressPolicy;
    quint64 m_progressGeneration = 0;
    ...
    bool m_hasBufferedPosition = false;
    int m_lastPresentedPositionSecond = -1;
