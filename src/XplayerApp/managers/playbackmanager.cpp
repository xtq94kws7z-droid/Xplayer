...
void PlaybackManager::init(XplayerCore* core)
{
    if (m_core == core) {
        return;
    }
                                     ...
                                     const QVariant& extraData)
{
    ...
    const bool independent = PlaybackWindowModeUtils::shouldUseIndependentWindow(
        ConfigStore::instance()->get<bool>(
            ConfigKeys::PlayerIndependentWindow,
            PlaybackWindowModeUtils::defaultIndependentWindowEnabled()));
    if (independent) {
...
void PlaybackManager::stopExternalPlayer()
{
    if (m_extProcess && m_extProcess->state() != QProcess::NotRunning) {
        ...
        const long long finalTicks =
            m_ipc ? m_ipc->currentPositionTicks() : 0;
        ...
        if (!m_externalTerminalHandled && m_core &&
            !mediaId.isEmpty() && !playSessionId.isEmpty()) {
            m_externalTerminalHandled = true;
            QCoro::connect(
                m_core->mediaService()->reportPlaybackStopped(
                    mediaId, mediaSourceId, finalTicks, playSessionId),
                this, []() {});
        }
...
{
    ++m_externalProgressGeneration;
    m_externalProgressPolicy.reset();
    m_externalTerminalHandled = true;
...
void PlaybackManager::dispatchExternalPlaybackProgress(
    const PlaybackProgressRequest& request)
{
    if (!m_core || !m_ipc || m_currentMediaId.isEmpty() ||
    ...
    const QString mediaSourceId = m_currentMediaSourceId;
    const QString playSessionId = m_currentPlaySessionId;
    const quint64 generation = m_externalProgressGeneration;

    QCoro::connect(
        m_core->mediaService()->reportPlaybackProgress(
            mediaId, mediaSourceId, request.positionTicks,
            request.isPaused, playSessionId),
        this, [this, generation]() {
            if (generation != m_externalProgressGeneration) {
                return;
            }

            const auto next = m_externalProgressPolicy.complete();
            if (next.has_value()) {
                dispatchExternalPlaybackProgress(*next);
            }
        });
}
...
#ifdef Q_OS_WIN
        return QString("\\\\.\\pipe\\xplayer-mpv-%1").arg(pid);
#else
        return QString("/tmp/xplayer-mpv-%1.sock").arg(pid);
#endif
    case PlayerType::VLC: {
                                            ...
                                            const QString& playerPathOverride)
{
    ...
    connect(m_progressTimer, &QTimer::timeout, this, [this]() {
        ...
        const PlaybackProgressRequest request{
            currentTicks, false, false};
        const auto dispatch =
            m_externalProgressPolicy.enqueue(request);
        if (dispatch.has_value()) {
            dispatchExternalPlaybackProgress(*dispatch);
        }
    });
    ...
    connect(process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
        ...
        if (m_externalTerminalHandled || m_extProcess != process) {
            process->deleteLater();
            return;
        }
        m_externalTerminalHandled = true;
        ...
        if (!m_currentMediaId.isEmpty() && !m_currentPlaySessionId.isEmpty()) {
            ...
            QCoro::connect(
                m_core->mediaService()->reportPlaybackStopped(
                    m_currentMediaId, m_currentMediaSourceId, finalTicks,
    ...
    m_externalTerminalHandled = false;
    m_externalProgressPolicy.reset();
    ++m_externalProgressGeneration;
    process->start();
                                                ...
                                                const QVariant& extraData)
{
    ...
    playerWindow->setWindowOpacity(0.0);
    playerWindow->show();
    playerWindow->playLaunchTransition();
