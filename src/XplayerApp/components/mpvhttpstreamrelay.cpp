...
void MpvHttpStreamRelay::pumpReplyToSocket(QTcpSocket *socket)
{
    ...
    while (!it->pendingWrite.isEmpty() &&
           socket->bytesToWrite() < kSocketQueuedBytesHighWater)
    {
        const qint64 written = socket->write(it->pendingWrite);
        if (written <= 0)
        {
            break;
        }
        recordRelayedBytes(written);
        it->pendingWrite.remove(0, static_cast<int>(written));
    }

    if (!it->pendingWrite.isEmpty())
    {
        return;
    }
    ...
    if (!it->headOnly && reply->isOpen())
    {
        while (reply->bytesAvailable() > 0 && socket->bytesToWrite() < kSocketQueuedBytesHighWater)
        {
            ...
            {
                it->pendingWrite = written > 0
                                        ? chunk.mid(static_cast<int>(written))
                                        : chunk;
                break;
            }
    ...
    const bool allReplyDataDrained =
        it->pendingWrite.isEmpty() &&
        (it->headOnly || !reply->isOpen() || reply->bytesAvailable() == 0);
