...
void MpvWidget::shutdown() {
    ...
    m_renderUpdateQueued.storeRelease(0);
...
void MpvWidget::onMpvRenderUpdate(void *ctx) {
    ...
    if (!self->m_renderUpdateQueued.testAndSetOrdered(0, 1)) {
        return;
    }
...
void MpvWidget::loadMediaNow(const QString &url, const QString &serverId, bool wasPending) {
    ...
    PlaybackEngineProfile::LoadContext loadContext;
    loadContext.isHttpStream = isHttpStream;
    loadContext.usesRelay = usingRelay;
    loadContext.httpProxy = mpvProxyValue;
    loadContext.forceSeekable = forceSeekable;
    const PlaybackEngineProfile::LoadPolicy loadPolicy =
        PlaybackEngineProfile::buildLoadPolicy(loadContext);
    QVariantMap loadOptions = loadPolicy.options;
