...
Q_LOGGING_CATEGORY(lcWebSocketPayload, "xplayer.websocket.payload",
                   QtWarningMsg)

QString safeWebSocketUrlForLog(QString url)
{
    static const QRegularExpression apiKeyPattern(
        QStringLiteral("([?&]api_key=)[^&\\s]+"),
        QRegularExpression::CaseInsensitiveOption);
    url.replace(apiKeyPattern, QStringLiteral("\\1<redacted>"));
    return url;
}
...
void EmbyWebSocket::connectToServer()
{
    ...
    qDebug() << "[EmbyWebSocket] Connecting to:" << safeWebSocketUrlForLog(url);
    if (m_profile.ignoreSslVerification &&
        ...
        qWarning() << "[EmbyWebSocket] SSL certificate verification is DISABLED"
                   << "| url:" << safeWebSocketUrlForLog(url);
    }
...
void EmbyWebSocket::onSslErrors(const QList<QSslError>& errors)
{
    ...
    qWarning() << "[EmbyWebSocket] TLS validation errors"
               << "| url:" << safeWebSocketUrlForLog(buildWebSocketUrl())
...
void EmbyWebSocket::onTextMessageReceived(const QString& message)
{
    ...
    if (lcWebSocketPayload().isDebugEnabled()) {
        QString dataPreview;
        if (data.isArray()) {
            dataPreview = QString::fromUtf8(
        ...
        qCDebug(lcWebSocketPayload).noquote()
            << "[WS RAW] <<" << messageType << "| Data:" << dataPreview;
    }
